...
CREATE TABLE public.scheduled_content (
  ...
  CONSTRAINT scheduled_content_platform_check CHECK (platform IN ('Facebook', 'Instagram', 'LinkedIn', 'Twitter')),
  CONSTRAINT scheduled_content_status_check CHECK (status IN ('draft', 'scheduled', 'published', 'failed'))
);
